...
W1 = tf.Variable(tf.random_uniform([39, 200], -1, 1))
W2 = tf.Variable(tf.random_uniform([200, 400],-1, 1))
W3 = tf.Variable(tf.random_uniform([400,500],-1, 1))
W4 = tf.Variable(tf.random_uniform([500,600],-1, 1))
W5 = tf.Variable(tf.random_uniform([600,2],-1, 1))
...
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels = train_y, logits = L5))
...
d = zip(list1, list2)
with open('output_boundaries.csv', 'w',encoding = 'utf8') as myfile:
